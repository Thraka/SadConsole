...
namespace SadConsole.UI.Themes
{
    ...
    public class ProgressBarTheme : ThemeBase
    {
        ...
        public override void UpdateAndDraw(ControlBase control, TimeSpan time)
        {
            ...
            ColoredGlyph foregroundAppearance;
            ColoredGlyph backgroundAppearance;

            if (Helpers.HasFlag((int)progressbar.State, (int)ControlStates.Disabled))
            {
                foregroundAppearance = Foreground.Disabled;
                backgroundAppearance = Background.Disabled;
            }

            else if (Helpers.HasFlag((int)progressbar.State, (int)ControlStates.MouseLeftButtonDown) ||
                     Helpers.HasFlag((int)progressbar.State, (int)ControlStates.MouseRightButtonDown))
            {
                foregroundAppearance = Foreground.MouseDown;
                backgroundAppearance = Background.MouseDown;
            }

            else if (Helpers.HasFlag((int)progressbar.State, (int)ControlStates.MouseOver))
            {
                foregroundAppearance = Foreground.MouseOver;
                backgroundAppearance = Background.MouseOver;
            }

            else if (Helpers.HasFlag((int)progressbar.State, (int)ControlStates.Focused))
            {
