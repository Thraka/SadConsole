...
namespace SadConsole
{
    ...
    {
        const char SpaceCharAlternative = 'a';
        ...
        {
            int spaceWidth = drawFont.IsCharacterSupported(' ') ? drawFont.GetCharacter(' ').Width : drawFont.GetCharacter(SpaceCharAlternative).Width,
                textLength = 0,
        ...
        public static void PrintTheDraw(this ICellSurface cellSurface, int x, int y, string text, TheDrawFont drawFont)
        {
            ...
            foreach (var item in text)
            {
                ...
                else if (item == ' ' && drawFont.IsCharacterSupported(SpaceCharAlternative))
                {
                    xPos += drawFont.GetCharacter(SpaceCharAlternative).Width;
                }
            }
        }
    }
