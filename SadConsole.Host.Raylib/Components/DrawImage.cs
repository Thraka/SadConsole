...
public class DrawImage : RenderComponent, IDisposable
{
    ...
    public override void Render(IScreenObject host, TimeSpan delta)
    {
        if (PositionMode == PositionModes.Cells)
            GameHost.Instance.DrawCalls.Enqueue(new DrawCalls.DrawCallTexture(_image.Texture, host.AbsolutePosition + ((IScreenSurface)host).Font.GetWorldPosition(PositionOffset, ((IScreenSurface)host).FontSize)));
        else
            GameHost.Instance.DrawCalls.Enqueue(new DrawCalls.DrawCallTexture(_image.Texture, host.AbsolutePosition + PositionOffset));
    }
