        ...
        ImGui.Begin(GuiDockspace.ID_CENTER_PANEL, ImGuiWindowFlags.NoCollapse | ImGuiWindowFlags.NoTitleBar);
        {
            if (ImGui.BeginTabBar("preview_tabs", ImGuiTabBarFlags.NoCloseWithMiddleMouseButton))
            {
                if (ImGui.BeginTabItem("SadConsole Output", ImGuiTabItemFlags.NoCloseWithMiddleMouseButton))
                {
                    ...
                    {
                        ImTextureRef texture = renderer.BindTexture(SadConsole.Host.Global.RenderOutputTexture);
                        ...
                        {
                            ImGuiSC.DrawTexture("output_preview_image", true, _scOutputMode, texture, new Vector2(Host.Global.RenderOutputWidth, Host.Global.RenderOutputHeight), out _, out _);
                ...
                if (ImGui.BeginTabItem("Item Preview", ImGuiTabItemFlags.NoCloseWithMiddleMouseButton))
                {
                    ...
                    if (GuiState._selectedScreenObject is IScreenSurface surface)
                    {
                        ...
                        {
                            ImTextureRef targetTexture = renderer.BindTexture(gameTexture.Texture);
                            ...
                            if (GuiState._selectedScreenObjectState.SurfaceState.RenderSteps.Length > 2)
                            {
                                ...
                                {
                                    targetTexture = renderer.BindTexture(((Host.GameTexture)GuiState._selectedScreenObjectState.SurfaceState.RenderSteps[GuiState._selectedScreenObjectState.SurfaceState.RenderStepSelectedItem].CachedTexture).Texture);
                                }
                            }
                            ...
                            {
                                ImGuiSC.DrawTexture("output_preview_surface1", true, _surfaceOutputMode, targetTexture, new Vector2(gameTexture.Width, gameTexture.Height), out var isActive, out var isHovered);
