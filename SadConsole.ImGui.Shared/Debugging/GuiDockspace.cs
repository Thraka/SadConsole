...
public class GuiDockspace : ImGuiObjectBase
{
    ...
    public unsafe override void BuildUI(ImGuiRenderer renderer)
    {
        ...
        if (!_runOnce)
        {
            ...
            new DockBuilder(idRootDockspace)
                .Window(ID_CENTER_PANEL)
                .DockLeft(0.3f)
                    .Window(ID_LEFT_PANEL)
                    .Done()
                .DockRight(0.3f)
                    .Window(ID_RIGHT_PANEL)
                    .Done();
        }
    }

    public unsafe void BuildUI2(ImGuiRenderer renderer)
    {
        ImGuiViewportPtr viewport = ImGui.GetMainViewport();
        ImGui.SetNextWindowPos(viewport.Pos);
        ImGui.SetNextWindowSize(viewport.Size);
        ...
        if (!_runOnce)
        {
            ...
            ImGuiP.DockBuilderSplitNode(idRootDockspace, ImGuiDir.Left, 0.3f, &idLeftPanel, &idCenterPanel);
