...
namespace SadConsole.Components
{
    ...
    public class DrawImage : RenderComponent, IDisposable
    {
        ...
        /// </remarks>
        public Point PositionOffset { get; set; } = Point.Zero;
        ...
        public override void Render(IScreenObject host, TimeSpan delta)
        {
            if (PositionMode == PositionModes.Cells)
                GameHost.Instance.DrawCalls.Enqueue(new DrawCalls.DrawCallTexture(_image.Texture, (host.AbsolutePosition + ((IScreenSurface)host).Font.GetWorldPosition(PositionOffset, ((IScreenSurface)host).FontSize)).ToVector2i()));
            else
                GameHost.Instance.DrawCalls.Enqueue(new DrawCalls.DrawCallTexture(_image.Texture, (host.AbsolutePosition + PositionOffset).ToVector2i()));
        }
