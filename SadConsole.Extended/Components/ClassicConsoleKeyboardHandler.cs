...
public class ClassicConsoleKeyboardHandler : KeyboardConsoleComponent
{
    ...
    ///
    /// If the host is a <see cref="Console"/>, it uses the <see cref="_attachedCursor"/> property, caching reference to it.
    /// You can't change the cursor reference unless you re-add this component. If the host is a <see cref="IScreenObject"/>,
    /// the first instance of a cursor in the <see cref="IComponentHost.SadComponents"/> collection is used.
    ...
    {
        if (!_isReady || _attachedCursor == null || _surface == null) { handled = false; return; }
