    ...
    public class ControlsConsole : ScreenSurfaceRenderer
    {
        ...
        public override void Render(IScreenSurface screen)
        {
            ...
            if (screen.Tint.A != 255)
            {
                ...
                GameHost.Instance.DrawCalls.Enqueue(new DrawCalls.DrawCallTexture(BackingTextureControls, new Vector2(screen.AbsoluteArea.Position.X, screen.AbsoluteArea.Position.Y), _finalDrawColor));
        ...
        public override void Refresh(IScreenSurface screen, bool force = false)
        {
            ...
            if (BackingTexture == null || screen.AbsoluteArea.Width != BackingTexture.Width || screen.AbsoluteArea.Height != BackingTexture.Height)
            {
                ...
                BackingTextureControls?.Dispose();
                BackingTextureControls = new RenderTarget2D(Host.Global.GraphicsDevice, screen.AbsoluteArea.Width, screen.AbsoluteArea.Height, false, Host.Global.GraphicsDevice.DisplayMode.Format, DepthFormat.Depth24);
                force = true;
            }
