...
public class KeyValuePairEditorWindow
{
    ...
    private class Instance : ImGuiObjectBase
    {
        ...
        public override void BuildUI(ImGuiRenderer renderer)
        {
            if (_firstShow)
            {
                ImGui.OpenPopup("Key-Value Pair Editor"u8);
                _firstShow = false;
            }

            ImGui.GetStyle().GetSpacing(out Vector2 framePadding, out Vector2 itemSpacing);

            ImGuiSC.CenterNextWindowOnAppearing(new Vector2(35 * ImGui.GetFontSize(), -1));
            if (ImGui.BeginPopupModal("Key-Value Pair Editor"u8, ImGuiWindowFlags.NoResize))
            {
                ImGui.Columns(2);
                ...
                ImGuiSC.ConfirmPopup("ConfirmDelete"u8, "Are you sure you want to delete this item?"u8, () =>
                {
                ...
                if (ImGuiSC.WindowDrawButtons(out bool result))
                {
                    if (result)
                        _onSaved(ToDictionary());
                    else
                        _onCancelled?.Invoke();
