...
namespace SadConsoleEditor.Panels
{
    ...
    class SelectionToolPanel : CustomPanel
    {
        ...
        private Func<SadConsole.Surfaces.Basic> saveBrushHandler;
        private Action<SadConsole.Surfaces.Basic> loadBrushHandler;
        ...
        public SelectionToolPanel(Action<SurfaceBase> loadBrushHandler, Func<NoDrawSurface> saveBrushHandler, Action stashHandler, Action restoreHandler, Action<NoDrawSurface> createTextResultHandler)
        {
            ...
            popup.Closed += (o2, e2) =>
            {
                if (popup.DialogResult)
                {
                    if (System.IO.File.Exists(popup.SelectedFile))
                    {
                        ...
                        //}
                        //else
                        loadBrushHandler((SadConsole.Surfaces.Basic)popup.SelectedLoader.Load(popup.SelectedFile));
                    }
                }
            };
            popup.CurrentFolder = lastFolder ?? Environment.CurrentDirectory;
            popup.FileLoaderTypes = new FileLoaders.IFileLoader[] { new FileLoaders.SadConsole.Surfaces.Basic(), new FileLoaders.TextFile(), new FileLoaders.Ansi() };
            popup.Show(true);
            popup.Center();
        }
