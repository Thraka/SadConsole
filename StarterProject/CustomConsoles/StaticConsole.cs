namespace StarterProject.CustomConsoles
{
    ...
    class StaticConsole: Console
    {
            ...
            : base(80, 25)
        {
            ...
            _cellData.Print(2, 1, "Text written with the default foreground and background");

            // Prints a string using the ColoredString class.
            _cellData.Print(2, 2, new ColoredString("Text using a colored string", Color.LightBlue, Color.Transparent, null));
            ...
            _cellData.Print(2, 4, colorString);

            // Prints a string, then changes the foreground of a single cell.
            _cellData.Print(2, 5, "Text written with defaults, then the foreground changed");
            _cellData.SetForeground(17, 5, Color.Yellow);

            // Prints a string, then changes the background of a single cell.
            _cellData.Print(2, 6, "Text written with defaults, then the background changed");
            _cellData.SetBackground(17, 6, Color.DarkGray);

            // Prints a string, then changes the foreground and background of a single cell.
            _cellData.Print(2, 7, "Text written with defaults, then the foreground and background changed");
            _cellData.SetBackground(17, 7, Color.White);
            _cellData.SetForeground(17, 7, Color.Black);

            // Prints a string, then changes the effect of the cell to a Blink effect.
            _cellData.Print(2, 8, "Text written with defaults, then the blink effect applied");
            _cellData.SetEffect(17, 8, new SadConsole.Effects.Blink() { BlinkSpeed = 0.5f });
